...
def mash(joystick, args):
    ...
    delay = args[2] or 0.1    # How long to wait between each button press.
    hold_for = args[3] or 0.1 # How long to hold each button for.
...
def tilt(joystick, args):
    ...
    degree = args[1] or 1      # How far(and which direction) to tilt it.
    hold_for = args[2] or 0.1  # How long to tilt axis.
...
def set_var(joystick, args):
    ...
    func = args[2] or None  # Register on_change callback.

    if key not in joystick.user_variables:
        joystick.user_variable_locks[key] = Lock()

    with joystick.user_variable_locks[key]:
        if func is not None:
        ...
        joystick.user_variables[key] = val
